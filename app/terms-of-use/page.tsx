...
import { Ship, Anchor, Mail, Phone, MapPin, ChevronDown } from "lucide-react";
import { SECTIONS, CONTACT, type BodyItem, type Section } from "@/data/terms-sections";
    ...
    <div className="min-h-screen bg-paper text-ink font-montserrat overflow-x-clip">
        ...
        <div className="max-w-5xl mx-auto pt-8 sm:pt-10 pb-2 sm:pb-8 relative">
          <div className="flex flex-col-reverse sm:flex-row sm:items-start sm:justify-between gap-4 px-4 sm:px-6 md:px-10">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-[13px] leading-6 sm:text-sm text-stone-800">Sahwa Coffee export dossier, read before you ship</p>
            </div>
          ...
          <p className="text-[13px] leading-6 sm:text-sm text-stone-800 mt-3 px-4 sm:px-6 md:px-10">
            We source, coordinate, and export. The farms are independent. That single fact shapes most of the clauses below.
          </p>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-10 py-4 sm:py-8 md:py-4 grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6 lg:gap-10">
        <nav className="hidden lg:block sticky top-24 self-start">
          <div className="text-xs font-semibold tracking-widest mb-4 text-muted font-mono">CARGO INDEX</div>
      ...
      <style jsx>{`
        ...
        .sail { animation: sail 5s ease-in-out infinite alternate; }
        @media (prefers-reduced-motion: reduce) {
          .fade-up, .sail { animation: none !important; }
        }
      `}</style>
    </div>
  );
