...
export default function Hero() {
  return (
    <section className="relative bg-cream overflow-hidden font-montserrat">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-28 lg:pt-32 pb-14 sm:pb-20 lg:pb-16">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 sm:gap-3 mb-8 mt-4 md:mt-0 bg-white/50 backdrop-blur-sm px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-amber-200/50 shadow-sm">
            ...
            <span className="text-[10px] md:text-xs tracking-[0.15em] sm:tracking-[0.25em] uppercase text-stone-600 font-medium whitespace-nowrap">
              Get in Touch
            </span>
            <span className="h-px w-4 sm:w-8 bg-linear-to-l from-transparent to-amber-400/60" />
          </div>

          <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-light tracking-tight text-stone-800 mb-6 leading-tight">
          ...
          <p className="max-w-xl mx-auto text-xs sm:text-sm md:text-sm lg:text-base text-stone-500 leading-6 md:leading-7 mb-8">
            Have a question about our coffee, shipping, or just want to say hello? 
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 md:mt-12">
            {CONTACT_LINKS.map((contact, index) => (
              <div key={index} className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
                <a 
                ...
                >
                  <svg className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 text-dark-brown group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
                  <span className="text-xs sm:text-sm font-medium text-stone-700 break-all">{contact.email}</span>
                </a>
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm font-medium text-stone-700 whitespace-nowrap">{contact.phone}</span>
                </a>
              </div>
            ))}
          </div>
