...
import { Ship, Anchor, MapPin, ChevronDown } from "lucide-react";
import { SECTIONS } from "@/data/terms-sections";
import SectionCard from "@/components/term-of-use/SectionCard";
import NavButton from "@/components/term-of-use/NavButton";

export default function TermsOfUse() {
  const [activeId, setActiveId] = useState(SECTIONS[0].num);
  ...
  const currentSection = SECTIONS.find((s) => s.num === activeId);

  return (
    <div className="min-h-screen bg-cream text-ink font-montserrat overflow-x-clip">
        ...
        <div className="max-w-5xl mx-auto pt-8 sm:pt-10 pb-2 sm:pb-8 relative">
          <div className="flex flex-col-reverse sm:flex-row sm:items-start sm:justify-between gap-4 px-4 sm:px-6 md:px-10">
            <div className="min-w-0">
              <h1 className="font-medium text-3xl sm:text-4xl md:text-6xl pb-1">
                Terms of Use
              </h1>
              <p className="mt-2 text-[13px] leading-6 sm:text-sm text-black">
                Sahwa Coffee export dossier, read before you ship
              </p>
            </div>

            <div className="fade-up fade-up-2 self-start -rotate-3 sm:-rotate-6 select-none opacity-80 border-2 border-stamp text-stamp px-3 py-1.5 sm:p-2 sm:px-3.5 shrink-0">
          ...
          <div className="fade-up fade-up-3 my-4 sm:mt-10 flex items-center gap-2 sm:gap-4 md:gap-6 px-4 sm:px-6 md:px-10">
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] md:text-xs tracking-widest font-mono shrink-0">
              <Anchor size={13} className="text-cargo shrink-0" />
              ETHIOPIA
            </div>

            <div className="relative flex-1 min-w-7 h-px border-t-[1.5px] border-dashed border-rule">
              <div className="sail absolute -top-2 left-0">
                <Ship size={16} className="text-stamp" />
              </div>
            </div>

            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] md:text-xs tracking-widest font-mono shrink-0">
              <span className="sm:hidden">DESTINATION</span>
              <span className="hidden sm:inline">DESTINATION MARKET</span>
              <MapPin size={13} className="text-cargo shrink-0" />
            </div>
          </div>

          <p className="text-[13px] leading-6 sm:text-sm text-stone-800 mt-3 px-4 sm:px-6 md:px-10">
            We source, coordinate, and export. The farms are independent. That
            single fact shapes most of the clauses below.
          </p>
        </div>
      </header>
      ...
      <div className="lg:hidden sticky top-20 md:top-0 z-20 bg-paper px-6">
        ...
        >
          <span className="truncate min-w-0">
            MANIFEST INDEX — {currentSection?.title}
          </span>
          <ChevronDown
            size={16}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-10 py-4 sm:py-8 md:py-4 grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6 lg:gap-10">
        <nav className="hidden lg:block sticky top-24 self-start">
          <div className="text-xs font-semibold tracking-widest mb-4 text-muted font-mono">
            CARGO INDEX
          </div>
        ...
        <main className="min-w-0">
          {SECTIONS.map((s) => (
            <SectionCard
              key={s.num}
              section={s}
              sectionRef={setSectionRef(s.num)}
            />
          ))}
        </main>
      </div>
